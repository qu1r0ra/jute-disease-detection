"""Hyperparameter tuning for all ML baseline models."""
...
from jute_disease.utils import WANDB_ENTITY, WANDB_PROJECT, get_logger
...
def run_tuning():
    ...
    for config in configs:
        with wandb.init(
            ...
            logger.info(
                f"Starting {config['search_type'].upper()} Search for {config['name'].upper()}..."
            )

            pipeline = Pipeline(
                [("scaler", StandardScaler()), ("classifier", config["model"])]
            )
            ...
            cv_results.to_csv(
                MODEL_DIR / f"{config['name']}_cv_results.csv", index=False
            )
            ...
            wandb.log(
                {
                    ...
                    "pr_curve": wandb.plot.pr_curve(
                        y_test, y_probas, labels=class_names
                    ),
                }
            )
            ...
            logger.info(
                f"Finished {config['name']}. Test Acc: {acc:.4f} | F1: {search.best_score_:.4f}"
            )
